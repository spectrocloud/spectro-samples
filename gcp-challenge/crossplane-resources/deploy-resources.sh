...
# Deploy the resources, one at a time. In this example, the output of one is not needed in the input
# of the next (the Cloud Run service has a fixed URL derived from the name we gave the app and the
# project number, which are both fixed). But doing it one at a time here simulates the way we'll
# eventually be deploying each resource.
